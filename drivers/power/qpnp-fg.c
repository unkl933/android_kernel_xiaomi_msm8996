...
enum {
	...
	FG_MISC				= BIT(8), /* Show misc FG info */
};
...
struct fg_chip {
	...
	struct work_struct	restore_temp_fg_work;
	...
	if (msoc == 0) {
		if (fg_reset_on_lockup && chip->use_vbat_low_empty_soc) {
			...
			if (!vbatt_low_sts)
				...
				return EMPTY_CAPACITY;
		} else if (chip->empty_restart_fg) {
			return EMPTY_REPORT_SOC;
		} else {
			return EMPTY_CAPACITY;
		}
...
static void restore_temp_fg_work(struct work_struct *work)
{
	struct fg_chip *chip = container_of(work, struct fg_chip,
					restore_temp_fg_work);
	int msoc, volt_uv, batt_temp;

	volt_uv = get_batt_voltage(chip);
	msoc = get_monotonic_soc_raw(chip);
	batt_temp = get_sram_prop_now(chip, FG_DATA_BATT_TEMP);
	/*
...
static bool is_first_est_done(struct fg_chip *chip)
{
	...
	if (fg_soc_sts & SOC_FIRST_EST_DONE && chip->empty_restart_fg)
...
wait:
	...
	rc = of_property_read_u32(profile_node, "qcom,cool-bat-decidegc",
					&cool_bat_decidegc);
	if (rc)
		pr_warn("couldn't find battery cool_bat_decidegc\n");

	if (cool_bat_decidegc > 0) {
		rc = set_prop_jeita_temp(chip, FG_MEM_SOFT_COLD, cool_bat_decidegc);
		if (rc)
			pr_warn("couldn't set cool_bat_decidegc to jeita_temp\n");
	}
...
static void empty_restart_fg_work(struct work_struct *work)
{
	...
	usb_present = is_input_present(chip);
...
static int fg_probe(struct spmi_device *spmi)
{
	...
	INIT_WORK(&chip->restore_temp_fg_work, restore_temp_fg_work);
	...
	pr_info("last soc %d\n", chip->last_soc);
	schedule_work(&chip->init_work);
	schedule_work(&chip->restore_temp_fg_work);
